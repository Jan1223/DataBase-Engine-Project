...
public class bplustree {
	...
	private void handleDeficiency(InternalNode in) {
		...
		// Borrow:
			...
			double borrowedKey = sibling.keys[0];
	...
	private void splitInternalNode(InternalNode in) {
		...
		double newParentKey = in.keys[midpoint];
		Double[] halfKeys = splitKeys(in.keys, midpoint);
	 ...
	 */
	private Double[] splitKeys(Double[] keys, int split) {

		Double[] halfKeys = new Double[this.m];
		...
		if (isEmpty()) {
			...
			if (!ln.insert(new DictionaryPair(key, value))) {
				...
				if (ln.parent == null) {
					...
					Double[] parent_keys = new Double[this.m];
	...
	private class InternalNode extends Node {
		 ...
		 */
		private InternalNode(int m, Double[] keys) {
			this.maxDegree = m;
		 ...
		 */
		private InternalNode(int m, Double[] keys, Node[] pointers) {
			this.maxDegree = m;
			this.minDegree = (int)Math.ceil(m/2.0);
	...
	public class DictionaryPair implements Comparable<DictionaryPair> {
		 ...
		 */
		public DictionaryPair(double key, String value) {
			this.key = key;
			this.value = value;
		}
...
//					case "Insert":
//						bpt.insert(Integer.parseInt(tokens[1]), tokens[2]);
//						break;
//
...
//							String value = bpt.search(Integer.parseInt(tokens[1]));
